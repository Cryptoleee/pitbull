// record_video.mjs <page.html> <out.mp4> <seconds> — records an HTML animation with Chromium (webm) and
...
const [html, out, secs] = process.argv.slice(2);
...
execFileSync('/usr/bin/ffmpeg', ['-y', '-v', 'error', '-i', path.join(dir, webm), '-t', String(secs), '-r', '30', '-c:v', 'libx264', '-preset', 'slow', '-crf', '17', '-pix_fmt', 'yuv420p', '-movflags', '+faststart', out]);
