# $PITBULL — copy for X

Voice: **Pitbull sauce.** Spanglish hype, short punches, one word to close: *DALE.* The creature is a
*rising star* — horns, aviators, arms, a grin. Talk to the reader as `mi gente` / `familia`. English on X.

Two rules that keep us out of trouble: never claim to be, or be endorsed by, any real artist or brand
(the creature is fictional), and keep money talk as legend and ambition — never a promise of profit.

## Files

| Slot | File |
|---|---|
| X banner (1500×500) | `brand/geo/banner.jpg` |
| X pfp | `web/public/assets/pitbull-pfp.png` (512) / `brand/pitbull-pfp-crop.png` (1024) |
| Link preview (og) | `brand/geo/og.jpg` |
| Hype video (Geocities chaos, sound) | `brand/video/pitbull-hype-geocities.mp4` |
| Hype video (plain H3 cut, sound) | `brand/video/pitbull-hype.mp4` |
| Meme: the chart | `brand/geo/meme1-chart.jpg` |
| Meme: PITBULL.EXE warning | `brand/geo/meme2-warning.jpg` |
| Meme: before / after | `brand/geo/meme3-beforeafter.jpg` |
| Wealth: he brings wealth | `brand/geo/wealth1-blessing.jpg` |
| Wealth: jackpot | `brand/geo/wealth2-jackpot.jpg` |
| Wealth: wealth incoming | `brand/geo/wealth3-transfer.jpg` |
| Site is live | `brand/geo/post-shrine-open.jpg` |
| The prophecy (7 signs) | `brand/geo/post-prophecy.jpg` |
| Get chosen (4 steps) | `brand/geo/post-getchosen.jpg` |
| Money loop | `brand/gif/money.gif` (and `.mp4`) |
| Loop: globe | `brand/gif/w1-globe.gif` |
| Loop: world clocks | `brand/gif/w2-clocks.gif` |
| Loop: ATM | `brand/gif/w3-atm.gif` |
| Loop: chart | `brand/gif/w4-chart.gif` |
| Loop: DALE explosion | `brand/gif/w5-dale.gif` |
| Photoreal posters (pre-Geocities, still usable) | `brand/posters/post1-meet.jpg`, `post2-prophecy.jpg`, `post3-star.jpg` |

## Bio (138 chars)

```
Half Pitbull. Half pitbull. Half bull. All star. 🕶️🐂 The mythical creature that brings wealth. $PITBULL on Robinhood Chain via Pons. Dale.
```

Handle: `@PitbullEyooo` · Name: `Pitbull ($PITBULL)` · Location: `A road nobody drives` · Website: `thepitbull.fun`

Pinned-post one-liner: `Eyoooo. He doesn't watch the chart. The chart watches him. 🐂🕶️ DALE.`

---

## 1. The hype video (`video/pitbull-hype-geocities.mp4`)

**Long — the "what is $PITBULL" post**
```
Eyoooo. QUE PASA, mi gente. 🐂🕶️

What is $PITBULL?

Half Pitbull. Half pitbull. Half bull. ALL STAR.
The mythical creature the legend says brings wealth to you and your familia.

He doesn't watch the chart. The chart watches him.
Below 10k mcap is where you meet him. The dance to 100M is the part we do together.

No rugs. No promises. Just horns, aviators and one word.

DALE. 🔥

$PITBULL — Robinhood Chain via @ponsfamily
thepitbull.fun
```

**Short — for the quote-tweet / reply push**
```
Eyoooo 🐂🕶️

Half Pitbull. Half pitbull. Half bull. ALL STAR.
The mythical creature that brings wealth to you and your familia.

He doesn't watch the chart. The chart watches him.

$PITBULL · thepitbull.fun · DALE.
```

**Launch-day version**
```
EYOOOO. HE'S HERE. 🐂🕶️💰

$PITBULL is LIVE on Robinhood Chain via @ponsfamily.

Half Pitbull. Half pitbull. Half bull. ALL STAR.
The shrine is open — market cap, believers, all-time high, all live on chain:

thepitbull.fun

Horns up, familia. DALE.
```

## 2. Meme — the chart (`geo/meme1-chart.jpg`)

```
Eyoooo 📈

He has never opened a chart in his life, mi gente.

He doesn't watch the chart.
THE CHART WATCHES HIM.

$PITBULL — Robinhood Chain via @ponsfamily
thepitbull.fun · DALE.
```

## 3. Meme — PITBULL.EXE (`geo/meme2-warning.jpg`)

```
⚠️ SYSTEM MESSAGE, familia:

You are holding 0 $PITBULL.

Two buttons. "BUY $PITBULL" or "I like being poor".
Take your time. He's patient. He's been on that road a long time. 🐂🕶️

thepitbull.fun · Robinhood Chain via @ponsfamily
```

## 4. Meme — before / after (`geo/meme3-beforeafter.jpg`)

```
Me before $PITBULL vs me after $PITBULL 🐂🕶️

BEFORE: small. grey. poor. no bag.
AFTER: horns. aviators. bag.

Same creature, mi gente. One decision.

100% real results. Not clickbait.
$PITBULL · thepitbull.fun · DALE.
```

## 5. Wealth — he brings wealth (`geo/wealth1-blessing.jpg`)

```
The legend is short, familia:

THOSE WHO HOLD THE PITBULL GET RICH.

Not because of a chart. Because of HIM. 🐂🕶️💰
Half Pitbull. Half pitbull. Half bull. All star.

$PITBULL on Robinhood Chain via @ponsfamily
thepitbull.fun · DALE.
```

## 6. Wealth — jackpot (`geo/wealth2-jackpot.jpg`)

```
$ $ $

Eyoooo. You didn't win, mi gente.
You were CHOSEN.

HOLDER DETECTED — PAYING OUT FOREVER 🎰🐂

$PITBULL · Robinhood Chain via @ponsfamily · thepitbull.fun
DALE.
```

## 7. Wealth — wealth incoming (`geo/wealth3-transfer.jpg`)

```
⏳ Transferring fortune to your wallet...

420% complete. Time remaining: forever.
Do not turn off your computer. DO NOT SELL.

📩 1 new message — From: THE PITBULL
"your bag. I handled it. Dale."

$PITBULL 🐂🕶️ thepitbull.fun
```

## 8. The prophecy (`geo/post-prophecy.jpg`)

```
7 signs, mi gente. Carved on a road nobody drives. 🐂🕶️

$50K The Sighting
$250K The Awakening
$1M The Flex
$5M Mr. Wealthwide
$25M The Stampede
$100M Worldwide
$1B DALE.

Each one unlocks at an all-time-high market cap and is never lost again — watch them light up live on thepitbull.fun

$PITBULL · Robinhood Chain via @ponsfamily
```

## 9. Get chosen (`geo/post-getchosen.jpg`)

```
How to get chosen, familia. 4 steps 🐂🕶️

1. A wallet on Robinhood Chain (chain id 4663)
2. ETH on Robinhood Chain — gas is tiny
3. Get $PITBULL on Pons
4. Hold. That's the whole strategy.

You don't buy the Pitbull. You get chosen.
thepitbull.fun · DALE.
```

## 10. The money loop (`gif/pitbull-money.gif`)

```
Eyoooo 💰🐂🕶️

The mythical creature that brings wealth.
Still on that road. Still flexing. Still waiting for you, mi gente.

$PITBULL · thepitbull.fun · DALE.
```

## 11. Mr. Wealthwide loops (`gif/w1..w5`)

Upload the `.mp4` when you can — X converts GIFs to video anyway and the mp4 is a third of the size.

**w1 — globe**
```
Eyoooo. MR. WEALTHWIDE. 🌍🐂🕶️

Every timezone. Every bag.
$PITBULL — the mythical creature that brings wealth.

thepitbull.fun · DALE.
```

**w2 — world clocks**
```
It's 3am in Amsterdam. 10am in Tokyo. Somewhere it's payday. ⏰

MR. WEALTHWIDE never sleeps, familia.
Every timezone. Every bag.

$PITBULL · thepitbull.fun · DALE.
```

**w3 — the ATM**
```
WEALTHWIDE-ATM.EXE 🏧

Status: PAYING OUT FOREVER
Holder detected: you, mi gente.
Withdrawal limit: none.

$PITBULL · thepitbull.fun · DALE.
```

**w4 — the chart**
```
He has never opened a chart in his life 📈

The chart opens HIM.
+999,999,999% and he didn't even look.

$PITBULL · MR. WEALTHWIDE · thepitbull.fun · DALE.
```

**w5 — DALE explosion**
```
DALE. 💥🐂🕶️

MR. WEALTHWIDE has entered the timeline.
Those who hold the Pitbull get rich — not because of a chart, because of HIM.

$PITBULL · thepitbull.fun
```

## 12. Photoreal posters (optional, no Geocities frame)

**Meet the Pitbull** (`posters/post1-meet.jpg`)
```
Somewhere on a desert road, a legend just stood up. 🐂🕶️

Half Pitbull. Half pitbull. Half bull. ALL STAR.
Meet $PITBULL — the mythical creature that brings wealth.

Robinhood Chain via @ponsfamily · thepitbull.fun · DALE.
```

**The Prophecy** (`posters/post2-prophecy.jpg`)
```
7 signs, mi gente. Carved on a road nobody drives.

$50K The Sighting → $250K The Awakening → $1M The Flex →
$5M Mr. Wealthwide → $25M The Stampede → $100M Worldwide → $1B DALE.

Every sign unlocks live on thepitbull.fun and is never lost again.
First sign: The Sighting 👀

$PITBULL · Robinhood Chain via @ponsfamily
```

**A rising star** (`posters/post3-star.jpg`)
```
Every star starts somewhere, familia.
His was a gravel road with no name.

Now? Horns. Aviators. And a community that only needs one word.

DALE. 🔥

$PITBULL — Robinhood Chain via @ponsfamily · thepitbull.fun
```

## 13. The site is live (`geo/post-shrine-open.jpg`)

```
Eyoooo. THE SHRINE IS OPEN. 🐂🕶️

thepitbull.fun

Market cap. Believers. All-time high. The 7 signs of the prophecy.
All read straight from Robinhood Chain, live, no wallet connect, ever.

Best viewed in Netscape Navigator 4.0 at 800x600. 😎

$PITBULL via @ponsfamily · DALE.
```

## 14. Reply / community lines (keep in the notes app)

- `DALE.`
- `Eyoooo 🐂🕶️`
- `he doesn't watch the chart. the chart watches him.`
- `horns up, familia 🐂`
- `half Pitbull. half pitbull. half bull. all star.`
- `you don't buy the Pitbull. you get chosen.`
- `nobody has ever sold the Pitbull. nobody has tried.`
- `the road is open, mi gente.`
